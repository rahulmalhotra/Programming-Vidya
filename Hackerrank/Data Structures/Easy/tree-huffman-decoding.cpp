...
*   Description:- Solution for HackerRank Tree: Huffman Decoding problem
...
void decode_huff(node * root, string s) {
    ...
    for(int i=0; i<length; i++) {
        ...
        // * If the current character is 0, navigate to the left child of the tree
        if(s[i]=='0') {
            currentNode = currentNode->left;
        }

        // * Otherwise, navigate to the right child of the tree
        else {
            currentNode = currentNode->right;
        }

        /*
        *   If the current node has data i.e.
        *   it is the leaf node. Add the data value to the result string
        *   and reset the current node to the root node of the tree
        */
        if(currentNode->data) {
