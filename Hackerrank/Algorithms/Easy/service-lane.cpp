...
*   Description:- Solution for HackerRank Service Lane problem
...
vector<int> serviceLane(int n, vector<int> width, vector<vector<int>> cases) {
    ...
    // * Iterating each case one by one
    for(vector<int> currentCase : cases) {

        /*
        *   Calculating the minimum width according to the
        *   entry and exit index of the current case
        */
        int index = currentCase[0], maxW = INT_MAX;
        while(index <= currentCase[1]) {
            maxW = min(maxW, width[index]);
            index++;
        }

        // * Adding the minimum width of the current case to the result vector
